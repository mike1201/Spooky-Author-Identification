...
# 1-(2)
def main(data_dir):
    f = open(data_dir + '/glove.6B.100d.txt', 'r', encoding='UTF-8')
    g = open(data_dir + '/glove.6B.100d_pickle', 'wb')
    ...
    for idx, line in enumerate(f.readlines()):
        word_split = line.split()  # [the, 0.354, 0.213 .... ] : binary
...
#def load_pickle():
#    g = open('data//glove.6B//glove.6B.50d_pickle', 'rb')
...
def word_id_convert(data_dir):
   ...
   # max_document_length = max([len(x.split()) for x in x_text])
    ...
    # load embedded word
    h = open(data_dir + '/glove.6B.100d_pickle', 'rb')
    ...
    for sentence in splitter: # sentence = [word1, word2, ... ]
        if len(sentence) < 60:
            sentence = sentence + ['.']*(60-len(sentence))
...
# 1-(3)
def test_data_id_convert(data_dir):

    # load question data
    g = open(data_dir + '/test_data_pickling', 'rb')
    x_text = pickle.load(g)
    max_document_length = 60
    ...
    # Save index_question
    word_indices = np.array(word_indices)
    word_index_pickle = open(data_dir + '/test_word_index_pickle', 'wb')
    pickle.dump(word_indices, word_index_pickle)
